...
from heppyy.util.mputils import pwarning


def get_LundDeclusteringType():
	j = fj.PseudoJet()
	ld = fj.contrib.LundDeclustering
	return type(ld)


class RTreeWriter(yasp.GenericObject):
	_fj_psj_type = type(fj.PseudoJet()) 
	_fj_psj_vector_type = type(vector[fj.PseudoJet]([fj.PseudoJet(1,1,1,1)]))
