...
namespace pythiafjtools{
    ...
    bool search_pythia(const Pythia8::Pythia & pythia, const int require_pid, const int daughters_size, int & satisfier_ip)
    {
    ...
    int update_hadronization(Pythia8::Pythia & pythia, const int require_pid/* = 0 */, const int daughters_size/* = -1*/)
    {
        ...
        if (require_pid == 0)  // Do not require any specific particle in the event
        {
            return hstatus;
        }

        // Otherwise, search for that specific particle
        int satisfier_ip = -1;
        bool pid_found = search_pythia(pythia, require_pid, daughters_size, satisfier_ip);

        int n_tries = 0;  int max_n_tries = 10;
        while (!pid_found && n_tries < max_n_tries)
        {
            //std::cout << "PYTHIA retrying hadronization step\n";
            pythia.event = savedEvent;
            hstatus = pythia.forceHadronLevel();
            n_tries++;
            pid_found = search_pythia(pythia, require_pid, daughters_size, satisfier_ip);
        }
        if (!pid_found)
        {
            std::cerr << "PYTHIA max number of hadronization attempts exceeded!\n";
        }
        return satisfier_ip;
    }
