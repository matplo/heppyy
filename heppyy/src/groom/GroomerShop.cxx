...
namespace contrib
{
	...
	{
			static LundGenerator lGen();
			static PseudoJet jet();
			static std::vector<LundDeclustering> lds = std::vector<LundDeclustering>();
			static LundDeclustering *zero_split()
			{
				return &lds[0];			
			}
	    // static LundDeclustering _zero_split = LundDeclustering();
			// static LundDeclustering _zero_split = &ld[0];
